 ...
 *
 * This compares two explicit y2 binaries on the same focused matrix rows. It is
...
export function loadAbConfigFromEnv(env: NodeJS.ProcessEnv = process.env): AbConfig {
  const baselineBin = requireAbsoluteExecutableBinary(
    env.Y2_AB_BASELINE_BIN ?? "",
    "baseline",
  );
  const candidateBin = requireAbsoluteExecutableBinary(
    env.Y2_AB_CANDIDATE_BIN ?? "",
    "candidate",
  );
  const model = env.Y2_AB_MODEL;
  if (!model) throw new Error("Y2_AB_MODEL is required for A/B runs");

  const trials = Number(env.Y2_AB_TRIALS ?? "3");
  if (!Number.isInteger(trials) || trials <= 0) {
    throw new Error(`Y2_AB_TRIALS must be a positive integer, got ${env.Y2_AB_TRIALS}`);
  }

  const outputDir =
    env.Y2_AB_OUTPUT_DIR ??
    mkdtempSync(join(tmpdir(), `y2-agent-quality-ab-${Date.now()}-`));

  return {
    baselineBin,
    candidateBin,
    model,
    rowIds: parseRowIds(env.Y2_AB_ROWS),
    trials,
    outputDir,
    workspaceRoot: env.Y2_AB_WORKSPACE_ROOT ?? REPO_ROOT,
    timeoutMs: Number(env.Y2_AB_TIMEOUT_MS ?? "300000"),
  };
}
...
export async function runAbTrial(
  ...
  const trialHome = mkdtempSync(join(tmpdir(), `y2-ab-${row.id}-${trialIndex}-${side}-`));
  ...
  try {
    ...
    if (json.model && json.model !== config.model) {
      score = {
        ...score,
        passed: false,
        reason: `${score.reason}; json.model ${json.model} did not match Y2_AB_MODEL ${config.model}`,
      };
    }
  } catch (err) {
    score = {
